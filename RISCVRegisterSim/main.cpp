...
int  main(int argc , char **argv)
{
    ...
    std::string workingoption = " Working";

    while ((opt = getopt(argc , argv , "dilmrz:")) !=  -1) // Use unix getopt to specify memory size 
    {
        switch (opt)
        {
            case 'd': workingoption = workingoption + "d"; // Allow user to choose memory size 
            break;
    ...
    std::cout << std::endl << workingoption << std::endl;
