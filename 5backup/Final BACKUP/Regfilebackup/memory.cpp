 ...
 void  memory::set16(uint32_t  addr , uint16_t  val)
{
	...
	mem[addr] = val; // Combined 16 bit values are now able to be stored */
        ...
        set8(addr, ( val & 0x000000ff ) ); // Set value at the address
        		
	set8(addr + 1, ( ( val >> 8 ) & 0x000000ff) );// little endian order requires the 2nd value to be stored 
...
void  memory::set32(uint32_t  addr , uint32_t  val)
{
	...
	set16(addr + 2, (( val >> 16 ) & 0x0000ffff ) ); // Increment the address by 2 in order to set the next
        //                                                  2 bytes 
